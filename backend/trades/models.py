...
class Trade(models.Model):
    ...
    slug = models.CharField(max_length=200, null=True) #CEMI-trades-by-varveler-on-jul-22-2021-b1ab8977
    ...
    def set_self_slug(self, small_uuid):
        #CEMI-trades-by-varveler-on-jul-22-2021-b1ab8977
        if not self.slug:
            self.slug = '{ticker}-trades-by-{username}-on-{date}-{uuid}'.format(
                                                        ticker = self.ticker,
                                                        username = self.user.user_name,
                                                        uuid = small_uuid,
                                                        date = datetime.datetime.strftime(self.start_time, '%b-%d-%Y').lower())
            self.save()
